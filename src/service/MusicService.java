...
//获取音乐
    public static Music getMusic(int music_id){
        ...
        Music music;
        music=dao.Music_db.getMusicFromDB(music_id);
        return music;
    }
    //获取用户
    public static User getUser(int user_id){
      User user;
       user=dao.Music_db.getUserFromDB(user_id);
        return user;
    }
    ...
    public static Group getGroup(int group_id) {
        Group group;
        group=dao.Music_db.getGroupFromDB(group_id);
        return group;
    }
    ...
    public static Project getProject(int project_id){
        Project project;
        project=dao.Music_db.getProjectFromDB(project_id);
        return project;
    }
    ...
    public static Music_comment getMusicComment(int music_id,int num,int type) {
        //int music_id 音乐ID int num 第几条评论 int type 排序类型，0 表示按热度（点赞数）排序，1 表示按时间排序

        Music music;
        music=getMusic(music_id);
        Music_comment comment=music.getMusic_comments(int num,int type);
